...
export function SpeciesDetailPanel({ species, variant = "panel", onUpdate }: Props) {
      ...
      <div className={className}>
        ...
        <div className="grid grid-cols-2 gap-4 mb-lg">
          <div className="p-md rounded bg-white/5">
            <div className="text-xs text-muted uppercase tracking-wider mb-1">种群数量</div>
  ...
  const chartData = [
    ...
    ...Object.entries(species.abstract_traits || {}).map(([k, v]) => ({ subject: k, A: v, fullMark: 1 }))
        ...
        {activeTab === 'traits' && (
          <div className="fade-in h-full flex flex-col">
             ...
             <div className="space-y-4">
               <h3 className="text-sm font-medium text-muted border-b border-white/10 pb-2">抽象特质</h3>
                 ...
                 {Object.entries(species.abstract_traits || {}).map(([key, value]) => (
                   <div key={key} className="flex items-center gap-3 text-sm">
                     ...
                     <div className="flex-1 h-2 bg-white/10 rounded-full overflow-hidden">
                       <div 
                         className={`h-full rounded-full ${value > 0.7 ? 'bg-warning' : value < 0.3 ? 'bg-info' : 'bg-success'}`} 
          ...
          <div className="fade-in space-y-6">
            {species.parent_code && (
              <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                ...
                <div className="text-xs text-muted mt-1">诞生于第 {species.created_turn || '?'} 回合</div>
              </div>
            )}
