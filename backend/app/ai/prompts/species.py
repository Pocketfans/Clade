...
=== 演化规则（50万年时间尺度 - 快速进化期）===
1. **属性权衡**：增强某属性时，应有其他属性下降作为代价（但净增益是允许的）
2. **显著变化**：单次变化幅度在 ±1.0 到 ±2.5 之间，高压力下可达 ±3.0
3. **压力响应**：变化应直接回应当前环境压力，越强压力=越大变化
4. **演化突破**：在关键压力下，净增益+1.0~+2.0是合理的（代表演化飞跃）
...
🚀 重要：早期时代是生命大爆发的黄金期！
- 太古宙/元古宙：生命形式快速多样化，基因突变活跃，演化潜力巨大
- 在环境压力下，进化可以非常迅速！显著的适应性变化是常态
- 简单生物可以快速演化出新结构（从休眠基因激活或新突变产生）
- 鼓励大胆的演化创新，这是生命史上最具创造力的时期！
...
- 指南：半径>0.3 应积极创新，允许2个新器官或特质突破；半径0.2-0.3 可有1个新器官；半径<0.2 以激活休眠基因为主
...
- 器官发育阶段：[原基]→[初级]→[功能]→[成熟]，高压力环境下可加速发育甚至跳跃！

优先在 activated_genes 和 trait_changes 中体现这些基因的激活
activated_genes 示例：["强化耐寒性", "原始眼点"] - 从上方推荐列表选择

=== 🧬 新休眠基因生成（演化种子库）===
必须为子代创造**2-5个新休眠基因**，为未来演化预埋潜力：
- 80%有益/中性 + 20%轻微有害（隐性，自然选择的原料）
- 潜力值5.0-10.0，可以超越当前物种能力（代表演化潜力）
- 显隐性比例：dominant 30% / codominant 35% / recessive 30% / overdominant 5%
- 鼓励创造"前瞻性基因"：如原始神经、原始消化腔、光感受器原基等
...
=== 🔥 压力驱动快速进化 ===
压力≤3 常规适应 | 压力4-6 显著适应(增益×1.5) | 压力7-8 激进演化(增益×2.0) | 压力≥9 生存突破模式
- 压力≥5：允许更大幅度的特质变化，单项可达+2.5
- 压力≥7：trait_changes 必须有显著增强(≥+2.0)，允许器官跳跃发育
- 压力≥9：增强项≥2.5，这是演化的关键时刻，大胆创新！
- 高压力=高回报：适应成功的物种将获得竞争优势
...
- ✅ 正确示例：{{"耐寒性": "+2.5", "代谢效率": "+1.0", "繁殖速度": "-1.5"}} (增+3.5, 减-1.5, 净变化+2.0，体现演化突破)
- 🚀 快速演化：环境压力驱动显著适应，每次分化可有明显进步！
...
⚠️ **trophic_level 必填！** 必须返回一个浮点数（如 2.5），范围在 {trophic_range} 内！
⚠️ trait_changes 必须有增有减，净变化可为正值（+1.0~+3.0的演化突破是允许的）！
⚠️ new_dormant_genes 必须生成2-5个新基因，为未来演化预埋种子！
⚠️ 植物（营养级<2）必须填写 life_form_stage 和 growth_form！
🚀 大胆设计演化特征，这是生命史上最具创造力的时期！
...
⚠️ **trophic_level 必填！** 宿主营养级 + 0.1（能量效率提升），必须返回浮点数
""",
...
4. **trophic_level 必填！** 必须返回浮点数（1.0-5.5），通常与父代相近(±0.5)
...
⚠️ **重要**: gains 总和不超过 {max_increase}。大胆设计适应性特征！每个演化特征增益 1.5-3.5 是合理的，展现显著的演化进步！

=== ⚖️ 边际递减与突破机会 ===
属性越高，新增益效率越低。但高压力环境下可突破限制！
建议：分散投资多个属性 OR 集中突破关键属性（触发突破效果）
🚀 早期生命演化期：边际递减效应减弱，鼓励大胆的属性提升！
...
=== 任务 ===
1) 输出显著的演化特征与增益，展现快速进化的魅力！系统会自动计算代价。
2) 为每个演化特征（器官/能力/形态）写出用途，在 gains 中给出数值（1.5-3.5是合理范围）。
   **⚠️ 所有 gains 数值之和必须 ≤ {max_increase}！**
3) 为每个物种生成拉丁学名、中文俗名、100-150字描述与分化事件摘要。描述中应体现显著的演化变化！
4) **必须返回 trophic_level（营养级）**：
   - 基于父代营养级（见摘要中的父代营养级信息）
   - 通常继承父代值，演化方向改变时可调整 ±0.5
   - 植物/藻类: 1.0-1.5, 食草动物: 2.0-2.5, 杂食: 2.5-3.5, 肉食: 3.0-5.0
5) **为每个物种生成2-5个新休眠基因**（new_dormant_genes）：
   - 基因应符合物种的生态位和演化方向，同时预埋未来演化潜力
   - 包含特质基因和器官原基，鼓励创造前瞻性结构（如原始神经索、感光细胞等）
   - 约80%有益/中性，20%轻微有害（遗传负荷，自然选择的原料）
   - 有害基因必须是隐性的
   - 潜力值可以较高（5.0-10.0），代表演化潜力而非当前能力
...
⚠️ **trophic_level 必填！** 必须是浮点数（如 2.3），基于父代营养级调整 ±0.5
...
⚠️ **trophic_level 必填！** 继承双亲中较高者±0.3，必须返回浮点数
...
【边际递减】属性越高，新增益效率越低（>50%上限效率降至60%，>70%降至30%）
【突破机会】接近阈值时可优先强化以触发特殊效果
""",
...
""",
    "constraints": """=== 约束条件 ===
1. 权衡：有增必有减，但净变化可为正值（+1.0~+2.0体现演化突破）。
2. 演化特征：必须针对该区域压力给出显著适应（器官/能力/形态），大胆创新！
...
{{
    ...
    "morphology_changes": {{"body_length_cm": 1.0}},
    "event_description": "...",
    "speciation_type": "{speciation_type}",
    "reason": "...",
    "organ_evolution": [...]
...
⚠️ **trophic_level 必填！** 必须返回浮点数（范围 {trophic_range}）
"""
}
