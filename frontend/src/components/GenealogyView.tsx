...
export function GenealogyView({ tree, loading, error, onRetry, onClose }: Props) {
  ...
  const [spacingX, setSpacingX] = useState(200); // Horizontal (Level) spacing
  const [spacingY, setSpacingY] = useState(60); // Vertical (Sibling) spacing
        ...
        <div className="genealogy-toolbar">
          ...
          <div className="view-mode-toggle">
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginRight: "1rem" }}>
              <span style={{ fontSize: "0.8rem", color: "rgba(255,255,255,0.6)" }}>间距:</span>
              <input 
                type="range" 
                min="100" 
                max="400" 
    ...
    >
      ...
      <div className="node-body">
        <div style={{ display: "flex", gap: "1rem", fontSize: "0.85rem", marginTop: "0.5rem" }}>
          <span>出生: T{node.birth_turn + 1}</span>
          {node.extinction_turn != null && <span>灭绝: T{node.extinction_turn + 1}</span>}
          <span>后代: {node.descendant_count}</span>
        </div>
        <div style={{ fontSize: "0.85rem", marginTop: "0.3rem", color: "rgba(226, 236, 255, 0.7)" }}>
          当前规模: {node.current_population.toLocaleString()} | 峰值: {node.peak_population.toLocaleString()}
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
              ...
              <InfoCard label="出生回合" value={`T${node.birth_turn + 1}`} />
              {node.extinction_turn != null && <InfoCard label="灭绝回合" value={`T${node.extinction_turn + 1}`} color="#f87171" />}
              <InfoCard label="当前规模" value={node.current_population ? node.current_population.toLocaleString() : "0"} />
              <InfoCard label="峰值规模" value={node.peak_population ? node.peak_population.toLocaleString() : "0"} color="#fbbf24" />
              <InfoCard label="后代数量" value={node.descendant_count ? node.descendant_count.toString() : "0"} />
              <InfoCard label="分化类型" value={node.speciation_type || "N/A"} />
            </div>
