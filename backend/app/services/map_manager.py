...
from ..schemas.responses import HabitatEntry, MapOverview, MapTileInfo
from .map_coloring import map_coloring_service, ViewMode
...
class MapStateManager:
    ...
    def snapshot_habitats(self, species_list: Sequence[Species], turn_index: int) -> None:
        print(f"[地图管理器] 开始记录栖息地快照，回合={turn_index}, 物种数={len(species_list)}")
        ...
        print(f"[地图管理器] 使用 {len(tiles)} 个地块计算栖息地")
        ...
        for species in species_list:
            ...
            suitability = [
                (tile, self._suitability_score(species, tile)) for tile in tiles
            ]
            ...
            for tile, score in top_tiles:
                ...
                habitats.append(
                    HabitatPopulation(
                        tile_id=tile.id or 0,
                        species_id=species.id or 0,
                        population=int(total * portion),
                        suitability=score,
    ...
    def get_overview(
        ...
        for tile in tiles:
            ...
            # 推断气候带（基于纬度）
            latitude_normalized = tile.y / (self.height - 1) if self.height > 1 else 0.5
            climate_zone = map_coloring_service.infer_climate_zone(latitude_normalized, max(0, relative_elev))
    ...
    def _smooth_noise(self, noise_grid: np.ndarray) -> np.ndarray:
        """简单的3x3均值滤波，使噪声稍微连续一些，形成自然的斑块"""
        h, w = noise_grid.shape
        new_grid = np.zeros_like(noise_grid)
        # 使用padding避免边界检查
        padded = np.pad(noise_grid, 1, mode='edge')
    ...
    def _generate_earth_like_height_map(self) -> np.ndarray:
        ...
        这样的好处是可以严格控制海陆比例（例如70%海洋）。
        """
        width, height = self.width, self.height
        # 初始化网格
        grid = np.zeros((height, width), dtype=float)
        
        # A. 随机放置 4-7 个“大陆核心” (高斯分布)
        # 模拟地球板块分布
        ...
        for _ in range(num_continents):
            ...
            # 大陆大小与强度
            radius_x = random.uniform(width * 0.1, width * 0.25)
            radius_y = random.uniform(height * 0.1, height * 0.3)
            strength = random.uniform(1.0, 2.0)
            
            # 在网格上应用高斯 (手动实现以避免复杂的scipy依赖)
            y_grid, x_grid = np.ogrid[:height, :width]
            
            # 处理 x 轴的循环边界 (地球是圆的)
            dx = np.abs(x_grid - cx)
            dist_x = np.minimum(dx, width - dx) # 取最短距离
            dist_y = np.abs(y_grid - cy)
            
            blob = strength * np.exp(-((dist_x**2)/(2*radius_x**2) + (dist_y**2)/(2*radius_y**2)))
            grid += blob

        # B. 叠加多层噪声以增加海岸线细节 (Fractal-like Noise)
        # 使用简单的正弦波叠加代替完整的 Perlin 噪声，减少依赖且足够用
        noise_strength = 0.4  # 噪声相对于大陆的权重
        for freq in [3, 7, 15]:
            ...
            # 正弦波叠加
            noise = np.sin(x_grid / width * freq * 2 * math.pi + phase_x) * \
                    np.cos(y_grid / height * freq * 2 * math.pi + phase_y)
            
            # 频率越高，振幅越小
            amplitude = noise_strength / (freq ** 0.6)
            grid += noise * amplitude

        # C. 转化为百分位排名 (0.0 - 1.0)
        # 这样我们可以说：前70%是海洋，后30%是陆地
        flat = grid.flatten()
        # argsort两次可以得到排名
    ...
    def _temperature(self, lat: float, elevation: float) -> float:
        """
        计算温度（°C），基于纬度和海拔
        - 赤道（lat=0.5）约30°C，极地（lat=0或1）约-5°C
        - 海拔每升高100m，温度降低0.6°C
        """
        # 基础温度（海平面）：赤道30°C，极地-5°C
        base = 30 - abs(lat - 0.5) * 70
        
        # 海拔修正：仅对陆地（elevation>=0）应用
        # 海洋温度不受"海拔"影响，只受纬度影响
        if elevation >= 0:
            # 每100m降低0.6°C
            altitude_adjustment = elevation * 0.006
